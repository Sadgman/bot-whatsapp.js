...
const grupo = require('./utils/groupTools.js');
...
client.on('message_create', async (message) => {
    ...
    */
    ...
    if(chat.isGroup && participantes(numero_cliente) && !(contact.id.user === numero_cliente) && !(Alastor_Number.includes(contact.id.user))){   
        let mmsg = message.body.toLocaleLowerCase();
        const links_baneados = await grupo.readBanLink();
        console.log(links_baneados);
...
/*         if (getAllInfoPlayer(contact.id.user).roles === "ladron") {
    ...
    if(getAllInfoPlayer(contact.id.user).roles === "ama"){
        ...
        if(update_dias(contact.id.user,day, 2) === false){
            update_info_player(contact.id.user, "dinero", await getAllInfoPlayer(contact.id.user).dinero + 10, true);
            update_info_player(await getAllInfoPlayer(contact.id.user).casado, "dinero", (await getAllInfoPlayer(await getAllInfoPlayer(contact.id.user).casado)).dinero - 10, true);
        ...
        if(partst.length === 2){
            ...
            if (message.hasQuotedMsg && (await message?.getQuotedMessage())?.hasMedia) {
                try {
                    ...
                    await chat.sendMessage(media, { sendMediaAsSticker: true, stickerAuthor: 'Por Alastor', stickerName: 'Osa Bot' });
                } catch (err) {
                ...
                try {
                    ...
                    media = new MessageMedia(d.mimetype, d.data, 'sticker');
                    await chat.sendMessage(media, { sendMediaAsSticker: true, stickerAuthor: 'ByAlastor', stickerName: 'Osa' });   
                } catch (err) {
                    message.reply('No se pudo crear el sticker');
                }
    ...
    if(message.body.toLocaleLowerCase().startsWith('osban ') && participantes(contact.id.user)){
        let link;
        console.log(message.hasQuotedMsg);
        if(message.hasQuotedMsg){
            console.log("message has quoted")
            const quotedMsg = await message.getQuotedMessage();
            link = quotedMsg.body.toLocaleLowerCase();
        }else{
            const parts = message.body.split(' ');
            link = parts.slice(1).join(' ');
        }
        grupo.addLinkBan(link)
        message.reply('baneado');
    }
    if(message.body.toLocaleLowerCase() === 'ossban' && participantes(contact.id.user) && message.hasQuotedMsg){
        const quotedMsg = await message.getQuotedMessage();
        const quotedContact = await quotedMsg.getContact();
        await quotedMsg.reply('Adiós mierda')
        chat.removeParticipants([quotedContact.id._serialized]);
    }
    if(message.body.toLocaleLowerCase() === 'oslinks' && participantes(contact.id.user)){
        let links = await grupo.readBanLink();
        let m= '';
        for(link of links){
            m += link + '\n\n';
        }
    ...
    if(message.body.toLocaleLowerCase().startsWith('desban ') && participantes(contact.id.user)){
        const parts = message.body.split(' ');
        const link = parts.slice(1).join(' ');
        grupo.removeLinkBan(link);
        message.reply('desbaneado');
    }
    // Economia ----------------------------------------------------
    ...
    function descargarM(stream, mensaje_error, nameM){
        ffmpeg()
            .input(stream)
            .audioBitrate(128)
            .save(`${nameM}.mp3`)
            .on('end', () => {
                const file = fs.readFileSync(`${nameM}.mp3`);
    ...
    if (message.body.toLowerCase().startsWith("m ")) {
            ...
            const cook = ytdl.createAgent(JSON.parse(fs.readFileSync("./assets/cookies/yt.json")));
            ...
            if(counterListRequestMusic <= 1){
                try {
                    ...
                    if (search.includes('https://youtu.be/')){
                        const nameM = './assets/audio/' + search.split('v=')[1];
                        if(fs.existsSync(`${nameM}.mp3`)){
                            const file = fs.readFileSync(`${nameM}.mp3`);
                            const media = new MessageMedia('audio/mp3', file.toString('base64'), 'audio');
                            chat.sendMessage(media, { quotedMessageId: message.id._serialized });
                            counterListRequestMusic = 0;
                        ...
                        stream = ytdl(search, { filter: 'audioonly', agent: cook });
                    ...
                    try {
                        ...
                        const nameM = '../assets/audio/' + x[0].url.split('v=')[1];
                        if(fs.existsSync(`${nameM}.mp3`)){
                            const file = fs.readFileSync(`${nameM}.mp3`);
                            const media = new MessageMedia('audio/mp3', file.toString('base64'), 'audio');
                            chat.sendMessage(media, { quotedMessageId: message.id._serialized });
                        ...
                        stream = ytdl(x[0].url, { filter: 'audioonly', agent: cook });
                        descargarM(stream, mensaje_error, nameM);
        ...
        if (chat.isGroup) {
            ...
            if (participantes(contact.id.user)) {
                ...
                message.reply(
                    ...
                    "Desactivar bot (db)\n" +
                    "Activar bot (ab)\n\n" +
                    "osban [link] banea un link\n" +
                    "desban [link] desbanea un link\n" +
                    "oslinks mirar los links baneados\n" +
